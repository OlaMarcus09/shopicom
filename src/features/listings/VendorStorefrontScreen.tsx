  ...
  return (
    <View style={styles.screen}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.grid}>{sellerListings.length ? sellerListings.map((item) => <Pressable key={item.id} onPress={() => onOpenProduct(item)} style={[styles.localCard, { width: cardWidth }]}><Image resizeMode="cover" source={{ uri: item.imageUrls[0] }} style={{ width: cardWidth, height: cardHeight - 42 }} /><Text numberOfLines={1} style={styles.productName}>{item.title}</Text><Text style={styles.productPrice}>GHS {item.price}</Text></Pressable>) : products.map((source, index) => <Pressable key={index} onPress={() => onOpenProduct()}><Image resizeMode="contain" source={source} style={{ width: cardWidth, height: cardHeight }} /></Pressable>)}</View>
      </ScrollView>
    </View>
  );
}
