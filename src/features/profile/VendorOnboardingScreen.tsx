...
export function VendorOnboardingScreen({ onBack, user }: { onBack: () => void; user: User }) {
  ...
  function continueToBusinessDetails() {
    const requiredPersonal = [fullLegalName, email, phone, dateOfBirth, region, cityTown, residentialAddress, digitalAddress, gender];
    if (requiredPersonal.some((value) => !value.trim())) {
      setStatus('Complete all required personal details first.');
      return;
    }
    setStatus('');
    setStep(2);
  ...
  function handleDateChange(event: DateTimePickerEvent, selectedDate?: Date) {
    ...
    if (event.type === 'set' && selectedDate) setDateOfBirth(formatDate(selectedDate));
  }
  ...
  async function submit() {
    const required = [businessName, category, subcategory, preferredContactMethod];
    if (required.some((value) => !value.trim())) { setStatus('Complete all required business details first.'); return; }
  ...
  return <View style={styles.screen}>
    <View style={styles.header}><Pressable hitSlop={10} onPress={step === 2 ? () => setStep(1) : onBack}><Text style={styles.back}>‹</Text></Pressable><Text style={styles.title}>Become a Vendor</Text><Text style={styles.stepCount}>{step}/2</Text></View>
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      <Text style={styles.intro}>{step === 1 ? 'Enter your personal information.' : 'Tell us about your business.'}</Text>
      {step === 1 ? <>
        ...
        <View style={styles.fieldGroup}><Text style={styles.label}>Date of birth*</Text><Pressable onPress={() => setShowDatePicker(true)} style={styles.select}><Text style={[styles.selectText, !dateOfBirth && styles.placeholder]}>{dateOfBirth || 'Select date of birth'}</Text><Text style={styles.calendar}>▣</Text></Pressable></View>
        {showDatePicker ? <DateTimePicker display="default" maximumDate={new Date()} mode="date" onChange={handleDateChange} value={parseDate(dateOfBirth)} /> : null}
        <SelectField label="Region*" onChange={setRegion} options={ghanaRegions} value={region} />
